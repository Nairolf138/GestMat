...
router.delete('/:id', auth(MANAGE_USERS), checkId(), async (req: Request, res: Response) => {
  const db = req.app.locals.db;
...
router.put('/me', auth(), updateUserValidator, validate, async (req: Request, res: Response, next: NextFunction) => {
  const db = req.app.locals.db;
