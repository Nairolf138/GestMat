...
export async function createStructure(db: Db, data: Structure): Promise<WithId<Structure>> {
...
export async function updateStructure(
  ...
  const res = await db.collection<Structure>('structures').findOneAndUpdate(
    { _id: new ObjectId(id) },
    { $set: data },
...
export function findStructureById(db: Db, id: string): Promise<Structure | null> {
  return db.collection<Structure>('structures').findOne({ _id: new ObjectId(id) });
}
